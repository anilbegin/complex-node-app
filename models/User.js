...
User.prototype.validate = function(){
    return new Promise(async (resolve, reject) => {
        ...
        if(this.data.password.length > 50){
            this.errors.push("Password cannot exceeed 50 characters")
        }
        ...
        //Only if username is vaild then check to see if it's already taken
        if(this.data.username.length > 3 && 
         ...
         //Only if email is vaild then check to see if it's already taken
         if(validator.isEmail(this.data.email)){
...
User.prototype.login = function(){
   ...
   })  //constructor hence Capital P in promise
